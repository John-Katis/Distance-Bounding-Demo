...
def ski_df(ver_host=HOST, ver_port=VERIFIER_PORT):
...
def ski_mf_prover_in_range(ver_host=HOST, ver_port=VERIFIER_PORT, atk_listen_port=ATTACKER_PORT):
...
def ski_mf_prover_out_of_range(ver_host=HOST, ver_port=VERIFIER_PORT, atk_listen_port=ATTACKER_PORT):
    st = load_json(f"{STATE_DIR}/ski_prover_state.json")
    n = st["n"]
    a1, a2 = st["a1"], st["a2"]
    ...
    for i in range(n):
        ...
        # Forward challenge to honest prover so they can compute the correct bit (late)
        pjs.send(vmsg)
        presp = pjs.recv()
...
def ski_tf(ver_host=HOST, ver_port=5001):
    st = load_json(f"{STATE_DIR}/ski_attacker_state.json")
    n = st["n"]
    a1, a2 = st["a1"], st["a2"]
...
if __name__ == "__main__":
    ...
    if args.mode == "df":
        ski_df(ver_port=args.ver_port)
    elif args.mode == "mf_in":
        ski_mf_prover_in_range(ver_port=args.ver_port, atk_listen_port=args.mf_listen)
    elif args.mode == "mf_out":
        ski_mf_prover_out_of_range(ver_port=args.ver_port, atk_listen_port=args.mf_listen)
    elif args.mode == "tf":
        ski_tf(ver_port=args.ver_port)